...
scope は知識の有効範囲を表す。秘匿度、公開可否、operation ごとの参照可否は scope rule と metadata の責務であり、scope に混ぜると propose workflow と validation が複雑になる。
...
- `private` を visibility として追加する: scope と visibility の責務分離は保てるが、現行 scope rule の用語と重複する。
...
- Positive: private 相当の知識を scope rule と redaction gate で扱う責務が明確になる。
- Positive: ADR 002 の scope model を維持できる。
- Negative: `private` という語を使いたい利用者には、`personal` と scope rule の組み合わせを説明する必要がある。
