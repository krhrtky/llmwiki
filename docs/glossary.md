...
知識が有効な範囲。初期モデルは `personal`、`team`、`org` の 3 層とする。

## personal

発見、仮説、個人メモ、個人/private 相当の知識を扱う scope。`private` は独立 scope ではなく、秘匿度や操作単位の参照可否は operation-aware access control で表現する。

## org

組織横断で再利用される正規知識スコープ。全情報を中央集権的に保存する場所ではなく、公式な語彙、制約、ポリシー、判断を置く層。

## propose

下位スコープの知識を、上位スコープ向けに抽象化・匿名化・根拠整理したうえで提出する操作。copy や backport ではない。
...
秘匿情報が上位スコープへ混入しないように、propose の前後で検査・匿名化・抽象化・拒否を行う必須ゲート。
