...
この文書で固定するのは、workflow と access decision の実装契約である。認可エンジンの評価順序、競合解決、検出方式の内部実装は後続 ADR に残す。
...
gate の出力契約は固定するが、検出方式は固定しない。初期実装は rule-based、LLM、DLP service、または組み合わせを許容する。
...
- policy object と decision log の最小項目は固定するが、実装上の評価順序、優先度、競合解決は未決事項とする。
...
- fixed minimum audit fields を超えて追加する監査項目。
