...
`CLI` と内部関数 API は、同じ意味の操作を提供する。
初期実装は [ADR 011](../adr/011-start-with-file-and-cli-first.md) に従い、file-first を前提にする。
初期 transport と storage layout の実装判断は [ADR 013](../adr/013-finalize-m3-cli-contract.md) に従う。
...
- `metadata store` は page owner、reviewer、citation metadata、confidence などの運用 metadata を保持する file-first store として扱う。
- store の直接編集権限は LLMWiki scope rule ではなく repository controls に委譲する。
...
- 初期 CLI は単一 binary `llmwiki` とする。
- command 形は `llmwiki <command>` とし、ここで固定する command 語彙は LLMWiki Core の知識操作 command に限る。対象は `ingest`、`query`、`related`、`file`、`lint`、`graph`、`propose`、`redact`、`export` であり、`llmwiki skill install` のような配布補助 command は含めない。
- subcommand package への分割は初期実装に含めない。
- CLI は人間向けの整形表示を持てるが、正式 output contract は JSON に統一する。

## API 形状

- 初期 API は HTTP API を必須にしない。
- CLI と同じ use case を呼ぶ内部関数 API を正本とする。
- transport adapter は後続実装とし、M3 では command meaning、入出力 contract、失敗語彙だけを固定する。
- HTTP、RPC、daemon 化、認証 middleware は M3 の範囲外とする。
...
| `content_level` | `metadata` / `summary` / `content`。scope evaluation の対象範囲粒度を表す。 |
| `subject_kind` | `user` / `agent` / `service_account` / `role`。scope rule 評価の対象。 |
| `subject_id` | scope rule 評価の対象 subject id。 |
...
| `query` | wiki を読んで回答する。 | `config`, `store`, `question`, `content_level`, `subject_kind`, `subject_id`, `retrieval_scope`。互換で `workspace_root`, `scope`。 | 回答、citations、confidence、filing candidate metadata、store metadata。 | 対象範囲から除外、根拠不足、入力が write side effect を要求した場合。 | 読み取り専用。query 自体は wiki store を更新しない。 |
| `related` | seed page から relation graph を辿って関連知識を返す。 | `workspace_root`, `seed`, `scope`, `operation`, `content_level`, `subject_kind`, `subject_id`, `retrieval_scope`。 | relation path、scope evaluation、関連ページ、説明付き候補。 | seed 不在、対象範囲から除外、relation traversal 不可、入力が write side effect を要求した場合。 | 読み取り専用。related 自体は wiki store を更新しない。 |
| `file` | query や ingest の結果を wiki 更新候補として整理する。 | `workspace_root`, `candidate`, `scope`, `owner`, `confidence`, `citations`。`team` / `org` では `reviewer` も必須。 | filing artifact、required metadata、review queue entry。 | citation 不足、scope 不明、owner 未指定、confidence 未指定、`team` / `org` で reviewer 未指定。 | wiki 本体を直接更新せず、review または lint 対象の candidate として保存する。 |
...
| `export` | bundle または scope を出力する。 | `workspace_root`, `paths` または `scope`, `content_level`, `subject_kind`, `subject_id`, `export_scope`。 | export artifact、manifest、対象一覧。 | 対象範囲から除外された内容を含む、未対応形式、workspace 外への出力。 | export は複製物を作るだけで、原本は変えない。 |
...
- 全 command の正式 output は JSON object とする。
- Markdown report は JSON output から生成する人間向け表示であり、互換性を保証する正式 transport には含めない。

## file-first 配置

- wiki 本文は OKF-compatible Markdown bundle に置く。
- frontmatter の `llmwiki` namespace は page とともに運ぶ正本 metadata を保持する。
- page 単位 metadata は本文 page に隣接する `*.llmwiki.yaml` に置く。
...
| Page | Metadata sidecar | Workflow sidecar |
| --- | --- | --- |
| `page.md` | `page.llmwiki.yaml` | `page.workflow.yaml` |
| `index.md` | optional | optional |
| `log.md` | optional | optional |

### sidecar の責務

- frontmatter には format profile が要求する最小情報と、`llmwiki.scope`、`llmwiki.lifecycle` など page とともに運ぶ正本 metadata を置く。
- `*.llmwiki.yaml` には owner、reviewer、risk_owner、confidence、citation metadata など肥大化しやすい運用 metadata を置く。
- `*.llmwiki.yaml` の `relations[]` には、relation type、target、必要に応じて `target_kind` を置く。`target_kind` は `doc`、`code`、`test`、`skill`、`command`、`generated`、`external` のいずれかで target 種別を表す。
- `*.workflow.yaml` には proposal、review、approval、rejection、hold reason など時系列で増える state を置く。
- writer は本文 `.md`、frontmatter、隣接 sidecar の未知 key を破壊してはならない。

## 内部関数 API の境界

- 各 command は内部では `CommandInput -> CommandResult` の純粋な use case として扱う。
- CLI は引数解釈と JSON serialization だけを担当し、command の意味判断は内部関数 API に寄せる。
- command 間で共有する失敗語彙は `exclude`、`hold`、`error` に固定する。
...
| `answer_query` | `query` | question、scope、content_level | answer、citations、confidence | 根拠が不足する場合 |
| `follow_related` | `related` | seed、scope、operation、subject metadata | related result、relation path、scope evaluation | scope 再評価が必要な場合 |
...
Codex Skill の配布導線は `llmwiki skill install --workspace-root . [--codex-home <path>]` とする。この command は LLMWiki Core の知識操作ではなく、repository 内の `skills/*/SKILL.md` を Codex skill directory へ配置する配布補助 command である。

### Distributed Codex Skills

| Codex Skill | Primary Commands | Responsibility |
| --- | --- | --- |
| `llmwiki` | none | skill suite の入口、SoT 読み順、停止条件、用途別 skill 選択 |
| `llmwiki-answer-query` | `query`, `related` | 質問、seed page、retrieval scope から read-only 取得を導出する |
| `llmwiki-ingest-source` | `ingest` | raw source から wiki candidate と evidence map を作る |
| `llmwiki-file-knowledge` | `file` | candidate を owner、reviewer、citation 付き filing artifact にする |
| `llmwiki-promote-knowledge` | `redact`, `propose` | lower scope から upper scope への昇格提案を redaction gate 付きで実行する |
| `llmwiki-maintain` | `graph`, `lint` | graph/lint 結果から保守判断と修正候補を作る |
| `llmwiki-export` | `export` | scope、content level、export scope に従って export する |

### Demo helper

Codex session の取り込み導線は `llmwiki codex-session import --workspace-root . [--sessions-root <path>] [--repo-root <path>] [--limit <n>]` とする。この command は LLMWiki Core の知識操作ではなく、ローカル Codex session 履歴を demo 用の personal wiki candidate に変換する補助 command である。

- `sessions-root` は read-only raw source root として扱い、raw JSONL を repository にコピーしない。
...
- `exclude`: scope rule により対象範囲へ含められない。
- `hold`: その場での実行は止めるが、再評価余地がある。
- `error`: 入力不正、parse failure、I/O failure などの技術的失敗。

`hold` は workflow や scope evaluation として扱い、必ずしも lifecycle の最終状態ではない。

## Related Requirements

- [Requirement 011: Graph and Relation](../requirements/011-graph-and-relation.md)
- [Requirement 012: Agent Skills](../requirements/012-agent-skills.md)
- [Requirement 013: API and CLI](../requirements/013-api-and-cli.md)
- [Requirement 014: Storage Boundary](../requirements/014-storage-boundary.md)
