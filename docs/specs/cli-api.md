...
- command 形は `llmwiki <command>` とし、command 語彙は `ingest`、`query`、`file`、`lint`、`graph`、`propose`、`redact`、`export` に固定する。
...
- graph edge の正本は Markdown link とし、typed relation の補助 metadata は [ADR 015](../adr/015-store-typed-relations-in-llmwiki-sidecar.md) に従い `*.llmwiki.yaml` に保存する。
- related retrieval は [ADR 020](../adr/020-use-hybrid-search-and-graph-traversal-for-related-retrieval.md) と [Related Retrieval 仕様](./retrieval.md) に従い、Post-M5 の拡張として扱う。M3 の command 語彙は変更しない。
