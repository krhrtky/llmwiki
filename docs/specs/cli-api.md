...
Codex Skill の配布導線は `llmwiki skill install --workspace-root . [--codex-home <path>]` とする。この command は LLMWiki Core の知識操作ではなく、repository 内の `skills/llmwiki/SKILL.md` を Codex skill directory へ配置する配布補助 command である。
