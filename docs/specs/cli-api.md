...
- `*.llmwiki.yaml` には owner、reviewer、risk_owner、confidence、citation metadata、access policy refs など肥大化しやすい運用 metadata を置く。
...
| `follow_related` | `related` | seed、scope、operation、subject metadata | related result、relation path、access decision | access 再評価が必要な場合 |
...
| `llmwiki-answer-query` | `query`, `related` | 質問、seed page、access policy から read-only 取得を導出する |
...
| `llmwiki-export` | `export` | scope、content level、access policy に従って export する |
