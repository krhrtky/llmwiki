...
- 現行 CLI vocabulary に合わせて `ingest`、`query`、`related`、`file`、`lint`、`graph`、`propose`、`redact`、`export` の最小 interface を定義する。
- file-first で実装できる範囲から開始する。

### M4: Workflow

- propose workflow、redaction gate、review、approval、rejection を実装する。
- access control は概念モデルから scope rule schema へ詳細化する。
